...
public class AccessFilterApi implements Filter {
    ...
    public void doFilter(ServletRequest servletRequest, ServletResponse servletResponse, FilterChain filterChain) throws IOException, ServletException {
        if (debugLog)
            System.out.println("AccessFilterApi.doFilter(" + ((HttpServletRequest) servletRequest).getServletPath() + ")");
    ...
    private boolean checkRights(HttpServletRequest req, UserRow user) {
        ...
        if (accAlways.contains(page)) {
            if (debugLog) System.out.println("AccessFilterApi.checkRights page=" + page + " PUBLIC");
        ...
        if (user != null) {
            ...
            if (roleRights != null) {
                ...
                if (pageAccess != null) {
                    if (debugLog)
                        System.out.println("AccessFilterApi.checkRights pageAccess.id=" + pageAccess.access_id);
...
//    }
    ...
    private boolean getAllActionAccess(String action, AccessRow access) {
        ...
        switch (action) {
            case "GET":
                return access.all_get;
            case "PUT":
                return access.all_put;
            case "POST":
                return access.all_post;
            case "DELETE":
                return access.all_delete;
        }
        return false;
    }
    ...
    private boolean getOwnActionAccess(String action, AccessRow access) {
        ...
        switch (action) {
            case "GET":
                return access.own_get;
            case "PUT":
                return access.own_put;
            case "POST":
                return access.own_post;
            case "DELETE":
                return access.own_delete;
        }
        return false;
    }
