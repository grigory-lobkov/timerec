...
public class RepeatConvert {
    ...
    public static void prepareInputList(List<RepeatRow> datas, UserRow user) {
        ...
        offsetList(datas, delta);
    }

    public static void prepareOutputList(List<RepeatRow> datas, UserRow user) {
        int delta = UserController.getTzOffsetSeconds(user);
        offsetList(datas, delta);
    }

    public static void offsetList(List<RepeatRow> datas, int delta) {
        if (delta == 0) return;
        for (RepeatRow r : datas) {
            r.time_from += delta;
            r.time_to += delta;
